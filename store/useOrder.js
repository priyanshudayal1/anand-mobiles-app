...
export const useOrderStore = create((set, get) => ({
    ...
    initiatePayment: (razorpayData) => {
        return new Promise((resolve, reject) => {
            if (!RazorpayCheckout) {
                const msg = "Razorpay module is not initialized. If you are on Expo Go, this native module will not work. Please use a Development Build.";
                console.error(msg);
            ...
            try {
                RazorpayCheckout.open(options)
                    .then(async (data) => {
                        ...
                        try {
                            ...
                            set({ error: "Payment verification failed", isProcessingPayment: false });
                            reject(verifyError);
                        }
                    })
                    .catch((error) => {
                ...
                const msg = "Razorpay is not available in this environment (likely Expo Go). Please use a native development build.";
                set({ error: msg, isProcessingPayment: false });
                reject(new Error(msg));
            }
        });
    ...
    placeOrderFromCart: async (addressId) => {
        ...
        try {
            ...
            const response = await api.post("/users/order/razorpay/create/", orderData);
